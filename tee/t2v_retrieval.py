...
from evaluation.eval import eval_epoch, my_eval_epoch
...
def main():
    ...
    parser = argparse.ArgumentParser('CLIP2Video on Text-Video Retrieval Task')
    ...
    with open(args.text_val_csv, 'w', newline='') as f:
        writer = csv.DictWriter(f, fieldnames=['sentence'])
        writer.writeheader()

    while True:
        sentence = input("Please enter some sentences (enter exit to exit): ")
        if sentence.lower() == "exit":
            break
        with open(args.text_val_csv, 'a', newline='') as f:
            writer = csv.DictWriter(f, fieldnames=['sentence'])
            writer.writerow({'sentence': sentence})

    # init test dataloader
    assert args.datatype in DATALOADER_DICT
    test_dataloader_text, test_length_text = DATALOADER_DICT[args.datatype]["test_text"](args, tokenizer)
    ...
    # prepare the text and video ids
    texts = []
    with open(args.text_val_csv, 'r', encoding='utf-8') as csvfile:
        reader = csv.DictReader(csvfile)
        for row in reader:
            texts.append(row['sentence'])
    if os.path.exists(args.text_val_csv):
        os.remove(args.text_val_csv)
    
    video_ids = [ f'video{i}' for i in range(10000)]
    text_ids = [i for i in range(len(top_k_indices_list))]

    for text_id, top_k_indices in zip(text_ids, top_k_indices_list):
        ...
        # save top_k_video_indices to txt file
        with open(os.path.join(folder_path, 'result.txt'), 'w') as txt_f:
            ...
            for index in top_k_indices:
                ...
                # copy video file
                video_file_path = os.path.join(f'../CLIP2Video/data/msrvtt_data/all_videos', video_id + '.mp4')
