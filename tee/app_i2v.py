...
batch_list_v, batch_visual_output_list = [], []
if args.use_db:
    batch_list_v, batch_visual_output_list = fetch_video_features()
if not batch_list_v and not batch_visual_output_list:
...
def predict():
    ...
    with torch.no_grad():
        ...
        for bid, batch in tqdm(enumerate(test_dataloader_image)):
            ...
            if args.distributed:
                dist.send(tensor=embedding, dst=1)  # Assuming worker rank is 
            ...
            encoder_outputs = torch.zeros_like(embedding)
            if args.distributed:
                dist.recv(tensor=encoder_outputs, src=1)
            else:
                encoder_outputs = model.clip.visual.Backbone_forward(embedding)
            ...
            logger.info("[可信执行环境] 计算检索图片的最终特征向量和目标视频的特征向量之间的相似度分数")
        sim_matrix = []
        # with profile():
    ...
    if args.use_db:
        ...
        # TODO fix issue, the db endpoint is not reachable
        if response.status_code == 200:
...
@app.route("/")
def index():
    return render_template("home.html")  # Renders the template above
...
def attest():
    ...
    try:
        ...
        files = {
            'report': (report_file, report_data),
            'certificate': (vlek_cert_file, cert_data)
        }
        if args.use_attest:
            ...
            status = '成功'
            logger.info("[客户端] 接收证明报告")
            details="""Reported TCB Boot Loader from certificate matches the attestation report.
...
if __name__ == "__main__":
    ...
    context = (os.path.join(MAIN_DIR, 'tee/keys/cert.pem'), os.path.join(MAIN_DIR, 'tee/keys/key.pem')) 
