...
class ResidualAttentionBlock(nn.Module):
    ...
    def forward(self, x_tuple):
        x, video_frame, mask = x_tuple
        x = x + self.attention(self.ln_1(x), mask)
        x = x + self.mlp(self.ln_2(x))
        return (x, video_frame, mask)
...
class Transformer(nn.Module):
    ...
    def forward(self, x, mask, video_frame=-1):
...
class CLIP(nn.Module):
    ...
    def __init__(self,
        ...
        # set row permutation matrix to global
        if self.text_row_permutation:
            self.p = None
            self.ip = None
    ...
    def encode_text_F1(self, input_ids, input_mask, segment_ids):
        ...
        # torch.save(embedding_output, 'embedding_output_en.pt')
        ...
        if self.text_row_permutation:
            # get row permutation matrix randomly every batch
            self.p, self.ip = getPi_Random(32)
            self.p, self.ip = self.p.half().to("cuda"), self.ip.half().to("cuda")
            embedding_output = torch.matmul(self.p, embedding_output)
            mask = torch.matmul(self.p, mask)
        ...
        return embedding_output, input_ids, input_mask, segment_ids, mask

    def encode_text_Backbone(self, embedding_output, per_mask):
        embedding_output = embedding_output.permute(1, 0, 2)  # NLD -> LND
        encoder_outputs = self.transformer(embedding_output, per_mask)
    ...
    def encode_text_F2(self, sequence_output, text):
        ...
        # row depermuation
        if self.text_row_permutation:
            sequence_output = torch.matmul(self.ip, sequence_output)

        # torch.save(sequence_output, 'sequence_output_en.pt')
